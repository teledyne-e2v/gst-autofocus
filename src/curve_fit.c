...
double min_double_arr(double *arr,int size);
...
{
  //const size_t n = 13;  /* number of data points to fit */
  ...
  for (i = 0; i < n; ++i)
    {
      fit_data.t[i] = PDAs[i];
      fit_data.y[i] = sharpness[i] - (min_double_arr(sharpness,n));
    }
  ...
  //free(fit_data.t);
  //free(fit_data.y);
  int res = (int) (gsl_vector_get(x, 1)*800);
  gsl_vector_free(f);
  gsl_vector_free(x);
  printf("apres3\n");
  return res;
